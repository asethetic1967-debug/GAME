...
function ensureFullWorldState(playerState) {
    ...
    if (ws.survival_status === undefined) ws.survival_status = "NORMAL";
    ...
    if (ws.inventory === undefined) ws.inventory = playerState.inventory || [];
...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/api/survival/use-resource' && req.method === 'POST') {
        try {
            ...
            if (action === 'eat') {
                ...
                ws.stamina = Math.min(100, (ws.stamina || 0) + 15);
                if (hpBar) hpBar.value = Math.min(maxHp, (hpBar.value || 0) + 10);
                logMessage = isEn 
                    ? '🍖 Consumed 1 Food Ration (+35 Hunger, +15 Stamina, +10 HP).'
                    : isJa
                    ? '🍖 食料を1個消費しました（空腹度+35、スタミナ+15、HP+10）。'
                    : '🍖 食用了 1 份食物口糧（飽腹度 +35，精力 +15，生命值 +10）。';
                ...
                ws.stamina = Math.min(100, (ws.stamina || 0) + 10);
                logMessage = isEn 
                    ? '💧 Drank 1 Pure Water (+40 Hydration, +10 Stamina).'
                    : isJa
                    ? '💧 純水を1個消費しました（水分+40、スタミナ+10）。'
                    : '💧 飲用了 1 份純淨水（水分 +40，精力 +10）。';
    ...
    if (pathname === '/api/camp/action' && req.method === 'POST') {
        try {
            ...
            // 1. Check if player is in combat or extreme danger
            const isCombat = !!(
                playerState.combat_state || 
                ws.flags?.in_combat || 
                ws.flags?.combat_active ||
                ws.flags?.combat === 'active'
            );

            if (isCombat) {
                return sendError(res, 400, isEn 
                    ? '❌ Cannot rest or camp during active combat! Escape or defeat enemies first.' 
                    : isJa 
                    ? '❌ 戦闘中は休息や野営ができません！敵を倒すか逃走してください。'
                    : '❌ 戰鬥中無法露營或休整！請先擊敗敵人或脫離危險。');
            }
            ...
            if (action === 'rest') {
                // Short Rest: costs 5 hunger, 5 hydration -> gains 40 stamina, 15 HP
                ws.hunger = Math.max(0, (ws.hunger || 100) - 5);
                ws.hydration = Math.max(0, (ws.hydration || 100) - 5);
                ws.stamina = Math.min(100, (ws.stamina || 0) + 40);
                ...
                logMessage = isEn
                    ? `⛺ Short Rest completed. Restored +40 Stamina, +15 HP (Cost: -5 Hunger, -5 Hydration).${bonus}`
                    : isJa
                    ? `⛺ 軽い休息をとりました。スタミナ+40、HP+15回復（消費: 空腹度-5、水分-5）。${bonus}`
                    : `⛺ 在營地稍作休整，恢復了 40 點精力與 15 點生命值（消耗：飽腹度 -5，水分 -5）。${bonus}`;
                ...
                if ((ws.stamina || 0) < 15) {
                    return sendError(res, 400, isEn ? 'Not enough stamina (need 15 SP) to scavenge!' : isJa ? 'スタミナ不足（15 SP必要）のため採取できません！' : '精力不足（需要 15 點精力），無法進行搜救或採集！');
                }
                ws.stamina = Math.max(0, (ws.stamina || 0) - 15);
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    for (let j = 0; j < lootTypes.length; j++) {
                        ...
                        if (r <= cumulative) {
                            ...
                            found.push(isEn ? itemEn : isJa ? itemJa : itemZh);
                            break;
                        }
                    }
                }
                ...
                logMessage = isEn
                    ? `🏕️ Scavenged the camp perimeter (-15 SP). Found: ${found.join(', ')}!`
                    : isJa
                    ? `🏕️ キャンプ周辺を探索しました（-15 SP）。獲得: ${found.join('、')}！`
                    : `🏕️ 在營地周圍巡邏與搜集（消耗 15 精力），尋獲了：${found.join('、')}！`;
            } else {
    ...
    if (pathname === '/api/explore/scavenge' && req.method === 'POST') {
        try {
            ...
            const ws = playerState.world_state;
            const spCost = Math.max(5, Math.floor((scratchPercent || 50) / 10));
    ...
    if (pathname === '/api/explore/extract' && req.method === 'POST') {
        try {
            ...
            if (forcedEscape && lost.length > 0) {
                msg = isEn 
                    ? `🏃 Evacuated hastily! Brought out: ${summaryNames || 'None'}, lost ${lost.length} items during retreat.`
                    : isJa 
                    ? `🏃 慌てて撤退しました！持ち出し成功: ${summaryNames || 'なし'}、撤退中に${lost.length}個の物品を紛失。`
                    : `🏃 倉皇撤離！成功帶出：${summaryNames || '無'}，撤退時遺失了 ${lost.length} 件物品。`;
            } else {
                msg = isEn 
                    ? `🎉 Extracted safely with all ${secured.length} items added to your backpack!`
                    : isJa 
                    ? `🎉 安全に撤退完了！獲得した${secured.length}個の物品をインベントリに収納しました。`
                    : `🎉 安全撤離成功！所有 ${secured.length} 件搜刮戰利品已穩妥收入冒險背包！`;
            }
        ...
        try {
            ...
            if (playerState) {
                ...
                if (playerAction) {
                    ...
                    // Deduct stamina for active actions
                    playerState.world_state.stamina = Math.max(0, (playerState.world_state.stamina || 100) - 10);
